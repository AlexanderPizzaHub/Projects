...
t_val_vx1,t_val_vx2 = tools.from_numpy_to_tensor([plot_val_x1,plot_val_x2],dtype=dtype)

f = gt.data_gen_interior(np.concatenate([plot_val_x1,plot_val_x2],axis=1))

#t_ygt,t_ugt,t_pgt,t_yd = tools.from_numpy_to_tensor([y_gt,u_gt,p_gt,y_data],[False,False,False,False],dtype=dtype)
'''def plot_2D_scatter(net,colx,coly,path):
        values = net(colx,coly).detach().numpy()
        fig = plt.figure()
        ax = plt.axes(projection='3d')
        ax.scatter3D(colx, coly, values, c=values, cmap='Greens')
        plt.savefig(path)
        plt.close()'''

def plot_vecfield(colx,coly,normal,path):
     plt.scatter(colx,coly)
     plt.quiver(colx,coly,normal[:,0],normal[:,1])
     plt.savefig(path)
     plt.close()

def plot_2D_scatter(colx,coly,path):
        plt.scatter(colx,coly,s=1)
        plt.savefig(path)
        plt.close()

def plot_2D_with_boundary(bdrylists,colx,coly,path):
    plt.scatter(colx,coly,s=1)
    for bd_col in bdrylists:
        plt.scatter(bd_col[0],bd_col[1],s=1)
    plt.savefig(path)
    plt.close()

def plot_2D_scatter_with_vec(colx,coly,vcolx,vcoly,normal,path):
        plt.scatter(colx,coly)
        plt.scatter(vcolx,vcoly)
        plt.quiver(vcolx,vcoly,normal[:,0],normal[:,1])
        plt.savefig(path)
        plt.close()
