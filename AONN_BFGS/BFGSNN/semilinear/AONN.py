...
with open("../config.yml", "r") as stream:
    try:
        ...
        config = ymlfile['semilinear']['AONN']
...
def hook_y(optimizer,nploss):
    ...
    if epoch%val_int==0:
        ...
        with torch.enable_grad():
            loss,res,misfit = pde.pdeloss(y,tdx1,tdx2,labelx1,labelx2,pdedata_y,tbx1,tbx2,bdrydat_prim,bw)
...
def hook_p(optimizer,nploss):
    ...
    if epoch%val_int ==0:
        ...
        with torch.enable_grad():
            loss,apde,abc = pde.adjloss(p,y,tdx1,tdx2,labelx1,labelx2,pdedata_phi,tbx1,tbx2,bdrydat_adj,bw)
...
for epoch in range(max_iter_out):
    ...
    for e3 in range(1):
        if ld > 1e-4:
            ld = gamma*ld
        with torch.no_grad():
            u_step = u(tdx1,tdx2) - ld * (alpha*u(tdx1,tdx2)+p(tdx1,tdx2))
