...
with open("../config.yml", "r") as stream:
    try:
        ...
        config = ymlfile['steady_NS']['AONN_BFGS']
...
exppath = "results/AONN_BFGS/" + config['expname'] + "/"
...
bw,dw,mu,max_iter_out,max_iter_inner,ld,gamma,m = itemgetter('bw','dw','mu','out_iter','inner_iter','stepsize','decay_rate','memory')(config)
...
#For u, given c, it solves the primal pde
    ...
    with torch.no_grad():
        u_dat = u(tdx1,tdx2)
        yadj_dat = yadj(tdx1,tdx2)
        grad_J = alpha*u_dat - yadj_dat #duJ=u-yadj
