...
tbx,tdx,tdx1,tdx2,tbx1,tbx2,tcx1,tcx2 = tools.from_numpy_to_tensor([b_c,d_c,dx1,dx2,bx1,bx2,cx1,cx2],[False,False,False,False,False,False,False,False])
...
def closure_deform():
        ...
        loss = torch.mean(torch.pow(deform_out,2))
        loss.backward()
        return loss
...
#For u, given c, it solves the primal pde
    ...
    with torch.no_grad():
        ...
        current_deform = torch.concatenate([deform(tdx1,tdx2),deform(tbx1,tbx2)],dim=0)
        grad_J = - torch.concatenate([update_domain,update_bdry],dim=0)
        cc = bfgshist.update(current_deform,grad_J)
        #if cc is not None:
        #    info.curvcon.append(float(cc))
        update_deform = current_deform + ld * bfgshist.precondition(grad_J)
